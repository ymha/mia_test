"""
Membership inference attack on CNN classifier

This script demonstrates that attacks WORK on CNN classifiers without privacy protections.
"""
...
def extract_cnn_features(model, data_loader, device='cpu'):
    """
    Extract features from CNN classifier for membership inference
...
def prepare_cnn_attack_dataset(shadow_dir='cnn_shadow_models',
                               cnn_classifier_path='cnn_classifier.pth',
                               device='cpu'):
    """
    Prepare attack dataset from CNN shadow models
    """
    print("Preparing attack dataset from CNN shadow models...")
    ...
    shadow_files = sorted([f for f in os.listdir(shadow_dir) if f.startswith('cnn_shadow_') and f.endswith('.pth')])
    ...
    for shadow_file in shadow_files:
        ...
        member_features_by_class, _ = extract_cnn_features(shadow_model, member_loader, device)
        ...
        non_member_features_by_class, _ = extract_cnn_features(shadow_model, non_member_loader, device)
...
def evaluate_cnn_attack(attack_models, target_classifier,
                       cnn_classifier_path='cnn_classifier.pth',
                       device='cpu'):
    """
    Evaluate attack on CNN classifier
    """
    print("\n" + "="*70)
    print("EVALUATING ATTACK ON CNN CLASSIFIER")
    print("="*70)
    ...
    member_loader = DataLoader(member_dataset, batch_size=128, shuffle=False)
    member_features_by_class, _ = extract_cnn_features(target_classifier, member_loader, device)

    non_member_loader = DataLoader(cifar10_test_dataset, batch_size=128, shuffle=False)
    non_member_features_by_class, _ = extract_cnn_features(target_classifier, non_member_loader, device)
...
if __name__ == "__main__":
    print("This script will demonstrate that attacks WORK on CNN classifiers!")
    print("Use it to train and evaluate attacks on the vulnerable CNN.")
