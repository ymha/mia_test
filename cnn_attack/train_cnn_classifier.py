...
This creates a CNN classifier for CIFAR10 that is vulnerable to membership
inference attacks.
"""
...
def train_cnn_classifier(num_samples=10000, epochs=100,
                        batch_size=128, learning_rate=0.001,
                        weight_decay=1e-7,
                        device='cuda' if torch.cuda.is_available() else 'cpu',
                        save_path='cnn_classifier.pth'):
    """
    Train CNN classifier

    This classifier is vulnerable to membership inference because:
    - Trained for many epochs (allows memorization)
    ...
    Args:
        ...
        weight_decay: Learning rate decay (1e-7)
    ...
    # Training loop
    model.train()
    print(f"Training CNN classifier on {num_samples} CIFAR10 samples...")
    print(f"Architecture: 2 Conv+Pool layers, FC(128), Output(10)")
    print(f"Activation: Tanh, LR: {learning_rate}, Weight Decay: {weight_decay}")
    ...
    print(f"\n{'='*70}")
    print(f"CNN classifier training complete!")
...
if __name__ == "__main__":
    ...
    model, indices = train_cnn_classifier(
